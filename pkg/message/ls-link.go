...
func (p *producer) lsLink(link *base.LinkNLRI, nextHop string, op int, ph *bmp.PerPeerHeader, update *bgp.Update, isIPv6 bool, spf bool) (*LSLink, error) {
	...
	if lslink, err := update.GetBGPLSAttribute(); err == nil {
		...
		msg.MaxLinkBWKbps = lslink.GetMaxLinkBandwidthKbps()
		msg.MaxResvBWKbps = lslink.GetMaxReservableLinkBandwidthKbps()
		msg.UnResvBWKbps = lslink.GetUnreservedLinkBandwidthKbps()
		msg.TEDefaultMetric = lslink.GetTEDefaultMetric()
